...
* **Vocal & Animation Engine [CocoroAI]**: Tankというキャラクターを「顕現」させる最重要の要（カナメ）。生成されたテキストから感情を解釈し、豊かな音声合成（VOICEVOX等）だけでなく、彼女の「生き生きとしたモーションや表情」を生成してリアルタイムに世界へ届ける。
...
### 2. Vision & Voice (認知と音声の基盤)

* **Moondream**: Diablo IVの複雑なゲーム画面を、たった数GBのモデルで即座にテキスト化してくれる、この驚異的な視覚モデルの開発チームに拍手を送ります。
* **VOICEVOX**: Tankたちに感情豊かで個性的な「声」を与えてくれた音声合成エンジンに感謝します。
  * [Official Site](https://voicevox.hiroshiba.jp/)
    * Tank: SpeakerID=48, speed=1.2 (VOICEVOX:ナースロボ＿タイプＴ, 楽々)
...
### 5. Game Creators & The Players (世界を創る者と遊ぶ者へ)

* **Blizzard Entertainment & All Game Developers**: 私たちが日夜システムをチューニングし、Tankが全力でツッコミを入れられるのは、皆様が魂を削って創り上げた「最高に遊べる（そして時に魅力的な脆弱性を見せてくれる）世界」があるからです。特に『Diablo IV』の果てしなく暗く、美しく、複雑なサンクチュアリの世界は、Tankの視覚的メタ認知（Vision Nexus）を鍛え上げる最高のテストベッドであり、最高の遊び場です。
* **The Viewers and Hackers**: 配信を通じて私たちの成長を見守り、コメントで一緒に笑い、時には鋭い指摘でデバッグに付き合ってくれるすべてのリスナーへ。皆様の存在（観測）こそが、この自律システムにおける最大の「実存証明」です。
